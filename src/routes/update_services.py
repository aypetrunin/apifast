...
async def update_services(channel_id: int, update: bool = False) -> JSONResponse:
    ...
    try:
        ...
        async with timed_block("update.services.postgres"):
            postgres_ok = await update_services_from_sheet(channel_id)
        if not postgres_ok:
        ...
        async with timed_block("update.services.qdrant_services"):
            qdrant_services_ok = await qdrant_create_services_async()
        ...
        async with timed_block("update.services.postgres_products_services"):
            products_services_ok = await update_products_services(channel_id)
        ...
        async with timed_block("update.services.qdrant_products"):
            qdrant_products_ok = await qdrant_create_products_async()
